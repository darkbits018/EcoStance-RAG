-- Migration 005: Create quota tracking tables (PostgreSQL)
...
-- Insert default quotas for existing tenants (only if table is empty)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM tenant_quotas LIMIT 1) THEN
