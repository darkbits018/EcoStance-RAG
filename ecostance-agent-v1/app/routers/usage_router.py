...
async def get_usage_summary(
    ...
    """
    Get platform-wide usage summary for super admin Analytics page.
    ...
    **Requires super admin authentication**
    ...
    try:
        ...
        try:
            result = db.execute(
                text("SELECT COUNT(*) FROM api_usage WHERE timestamp >= :start AND timestamp <= :end"),
                {"start": start, "end": end}
            ).fetchone()
        ...
        try:
            result = db.execute(
                text("SELECT COUNT(*) FROM tenant_documents")
            ).fetchone()
            total_documents = result[0] if result else 0
        except:
            total_documents = 0
        
        # Total storage across all tenants (in GB)
        try:
            tenants = db.query(Tenant).all()
            total_storage_bytes = sum(
                (tenant.settings or {}).get("current_storage_bytes", 0) 
        ...
        # Active tenants (tenants with queries in date range)
        try:
            result = db.execute(
                text("""
            ...
            active_tenants = result[0] if result else 0
        except:
            active_tenants = 0
        
        # Average queries per tenant
        avg_queries_per_tenant = round(total_queries / active_tenants) if active_tenants > 0 else 0
        ...
        try:
            ...
            avg_response_time_ms = round(result[0]) if result and result[0] else 0
        except:
        ...
        return {
            ...
            "avg_queries_per_tenant": avg_queries_per_tenant,
