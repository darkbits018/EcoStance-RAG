...
def get_tenant_id_from_request(request: Request) -> str:
    ...
    # For now, use a default tenant (in production, extract from subdomain)
    # This should be replaced with proper tenant resolution
...
async def query_public_chat(
    ...
    try:
        ...
        # Query the first allowed KB (can be extended to query multiple)
        kb_id = allowed_kbs[0]
        ...
        assistant_message = service.add_message(
            ...
            tenant_id=tenant_id,
            role="assistant",
            content=rag_response.get("answer", "I'm sorry, I couldn't find an answer to your question."),
        ...
        return PublicChatQueryResponse(
            answer=rag_response.get("answer", "I'm sorry, I couldn't find an answer to your question."),
            sources=sources,
            session_id=request_data.session_id,
...
async def get_public_chat_config(
    ...
    try:
        ...
        config = service.get_config(tenant_id)
        if not config or not config.enabled:
            return PublicChatConfigDisabledResponse()
