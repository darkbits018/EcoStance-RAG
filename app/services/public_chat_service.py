...
class PublicChatService:
    ...
    def update_config(
        ...
        # Validate allowed_kbs exist
        if update_data.allowed_kbs:
            kb_count = self.db.query(TenantKnowledgeBase).filter(
                and_(
                    TenantKnowledgeBase.tenant_id == tenant_id,
                    TenantKnowledgeBase.kb_id.in_(update_data.allowed_kbs)
                )
            ).count()
            if kb_count != len(update_data.allowed_kbs):
                raise ValueError("One or more knowledge bases not found")
