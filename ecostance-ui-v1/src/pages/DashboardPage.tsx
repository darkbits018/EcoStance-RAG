...
import { quotaAPI, extendedMetricsAPI, knowledgeBaseAPI } from '../services/api';
...
const mockRecentActivity = [
  { id: 1, type: 'query', description: 'What is the capital of France?', timestamp: '2024-07-26T10:00:00Z', status: 'completed' },
  { id: 2, type: 'upload', description: 'Uploaded document: report_q3.pdf', timestamp: '2024-07-26T09:45:00Z', status: 'completed' },
  { id: 3, type: 'db_query', description: 'SELECT * FROM users', timestamp: '2024-07-26T09:30:00Z', status: 'completed' },
  { id: 4, type: 'job', description: 'Processing job: document_ingestion_123', timestamp: '2024-07-26T09:15:00Z', status: 'processing' },
  { id: 5, type: 'query', description: 'Define Ecostance RAG', timestamp: '2024-07-25T18:00:00Z', status: 'completed' },
  { id: 6, type: 'upload', description: 'Uploaded document: presentation.pptx', timestamp: '2024-07-25T17:30:00Z', status: 'completed' },
  { id: 7, type: 'db_query', description: 'UPDATE products SET price = 100', timestamp: '2024-07-25T16:00:00Z', status: 'completed' },
  { id: 8, type: 'job', description: 'Processing job: data_sync_456', timestamp: '2024-07-25T15:00:00Z', status: 'failed' },
  { id: 9, type: 'query', description: 'How to use the dashboard?', timestamp: '2024-07-24T11:00:00Z', status: 'completed' },
  { id: 10, type: 'upload', description: 'Uploaded document: image.png', timestamp: '2024-07-24T10:30:00Z', status: 'completed' },
];

const ActivityItem = ({ item }: { item: typeof mockRecentActivity[0] }) => {
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'db_query': return <Icons.Database className="h-4 w-4 text-purple-500" />;
  ...
  return (
    <div className="flex items-center py-2 border-b border-border last:border-b-0">
      ...
      <span className={`text-xs font-medium px-1.5 py-0.5 rounded-md ${getStatusColor(item.status)}`}>
...
const ChartPlaceholder = ({ title, description }: { title: string; description: string }) => (
  <Card className="w-full max-w-lg h-56 flex flex-col items-center justify-center bg-surface border-none shadow-none">
    <CardHeader className="p-4 pb-2">
      <CardTitle className="text-base font-semibold text-primary">{title}</CardTitle>
      <CardDescription className="text-xs text-text-secondary">{description}</CardDescription>
    </CardHeader>
    <CardContent className="p-4 pt-0 flex-1 w-full flex items-center justify-center">
      <div className="w-full h-full bg-gray-200/30 rounded-md flex items-center justify-center text-text-secondary/70 text-sm">
        Chart Placeholder
      </div>
    </CardContent>
  </Card>
...
const DashboardPage: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      console.log('🔍 Raw quota response:', quotaResponse);
      ...
      let kbCount = 0;
      try {
        const kbResponse = await knowledgeBaseAPI.list() as any;
        kbCount = Array.isArray(kbResponse) ? kbResponse.length : 0;
      } catch (err) {
        console.warn('Failed to load knowledge bases:', err);
      }

      // Fetch active alerts (optional - don't fail if alerts endpoint has issues)
      try {
        ...
        console.warn('Failed to load alerts (non-critical):', err);
        // Don't show error to user - alerts are optional
        setAlerts([]);
      }

      // Update overview data with safe fallbacks
      setOverviewData({
        knowledgeBases: kbCount,
        documentsUploaded: quotaData?.documents?.used || 0,
        storageUsedGB: (quotaData?.storage?.used_bytes || 0) / (1024 ** 3), // Convert to GB
  ...
  return (
    <main className="min-h-screen w-full bg-background flex flex-col items-center justify-start p-4 md:p-8 font-sans text-text">
      <div className="w-full max-w-6xl text-center">
        ...
        {/* Recent Activity Feed */}
        <section className="mb-8">
          <h2 className="text-xl font-semibold text-left mb-4 text-primary">Recent Activity</h2>
          <Card className="w-full max-w-4xl mx-auto bg-surface border-none shadow-none">
            <CardContent className="p-4">
              {mockRecentActivity.slice(0, 5).map((item) => ( // Displaying fewer items for a cleaner look
        ...
        {/* Usage Charts */}
        <section>
          <h2 className="text-xl font-semibold text-left mb-4 text-primary">Usage Analytics</h2>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <ChartPlaceholder title="Storage Usage Trend" description="Last 30 days" />
            <ChartPlaceholder title="Query Volume" description="Last 7 days" />
            <ChartPlaceholder title="API Calls" description="Per day" />
          </div>
        </section>
